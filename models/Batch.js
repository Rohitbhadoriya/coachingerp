...
batchSchema.pre('save', async function () {
    try {
        ...
        if (this.status !== 'cancelled') {
            ...
            else if (this.endDate && currentDate >= this.startDate && currentDate <= this.endDate) {
                this.status = 'active';
            } 
            else if (this.endDate && currentDate > this.endDate) {
                this.status = 'completed';
            }
        }
    } catch (error) {
        console.error("Batch Pre-save Error:", error.message);
        throw error;
    }
});
